            ...
            <!-- 3. Quick Transform (Assets) -->
             <section class="relative space-y-6 pt-8 border-t border-white/5 flex-grow pb-24 animate-in fade-in slide-in-from-bottom-6 duration-700 delay-200">
                ...
                <div v-if="result && result.assets" class="space-y-6">
                    ...
                    <button @click="copy(result.markdown)" class="w-full py-4 rounded-xl bg-accent text-black font-bold uppercase tracking-widest hover:brightness-110 active:scale-[0.98] transition-all shadow-[0_0_30px_rgba(204,255,0,0.15)] flex items-center justify-center gap-2">
                ...
                <!-- Skeleton -->
                <div v-else class="space-y-6 opacity-20">
                    <div class="h-40 bg-white/5 rounded-xl border border-white/5"></div>
                    <div class="space-y-2">
...
const activeStep = computed(() => {
    ...
    if (inputs.value.q1_core_what.trim().length > 2) return 1; // Product exists -> Step 1 (Draft)
    return 0; // Start
});
...
const generateFull = async (isRemix = false) => {
    ...
    try {
        ...
        const mappedResult = {
            ...
            assets: {
                oneLiner: res.oneLiner, // Direct from top-level
                heroHeadlines: res.headlines || [],
                positioning: res.positioning || "Positioning statement..."
            },
            debug: {
                ...res.debug,
        ...
        // Secondary Call: Positioning Generator (Async)
        // We do not await this to let UI render Vibe first, but Vue reactivity will update when it arrives.
        // Actually, for "Remix", users expect instant updates. For "Upgrade", maybe async is fine.
        // Let's await it to prevent "pop-in" or show a loading state for that section.
        // Given required speed "3 seconds", parallel is better.
        
        $fetch('/api/generate/positioning', {
            method: 'POST',
            // Update payload with potentially new product name if Demo endpoint changed it?
            // Demo endpoint returns 'res.identity.productName'. We should prefer that.
            body: {
                ...payload,
                knownProductName: res.identity.productName || payload.knownProductName
            }
        }).then(posRes => {
            if (result.value && result.value.assets) {
                // Use the structured positioning text
                result.value.assets.positioning = posRes.positioningDraft.positioning;
                // Merge debug info
                if (result.value.debug) {
                    result.value.debug.positioningDebug = posRes.debug;
                }
            }
        }).catch(err => {
